...
#项目名称
    ...
    #     assert text == "不能为空"
    # 借款期限为非整数
    def test_onlineapply_month11(self, anxiang_web):
        self.clear(anxiang_web)
        self.itemname(anxiang_web, "测试")
        self.itemcity1(anxiang_web)
        self.itemcity2(anxiang_web)
        self.count(anxiang_web, 50)
        self.month(anxiang_web, 100.1)
        self.percent(anxiang_web, 3)
        ...
        assert  WebDriverWait(anxiang_web.driver, 10).until(
            EC.visibility_of_element_located((By.CSS_SELECTOR,
                                               "#applyForm > div > div.lg-item.verify-group.fn-clear.has-error > div > label"))).text == "请输入大于0的整数"
...
#成功申请
    def test_onlineapply_success(self,anxiang_web):
        self.clear(anxiang_web)
        ...
        self.detail(anxiang_web,1.1)
        anxiang_web.driver.implicitly_wait(10)
        WebDriverWait(anxiang_web.driver, 10).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, "#applyForm > div > div:nth-child(7) > input"))
        ).click()
        WebDriverWait(anxiang_web.driver, 10).until(
            EC.presence_of_element_located((By.CSS_SELECTOR,"#mlayout > div.header.fn-clear > div.header-top > div > div.sub-nav.fr > li.last > span > a"))).click()
